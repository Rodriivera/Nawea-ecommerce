-- 012: corrección de variables de iteración en create_order_with_reservation
...
begin
  ...
  -- Soporta p_items como { "lines": [...] } o directamente como array [...]
  if jsonb_typeof(p_items) = 'array' then
    v_lines := p_items;
  elsif jsonb_typeof(p_items) = 'object' and p_items ? 'lines' then
    v_lines := p_items -> 'lines';
  else
    raise exception 'Formato de ítems inválido';
  end if;
  ...
  for v_elem in select jsonb_array_elements(v_lines)
  loop
    v_product_id := (v_elem ->> 'product_id')::uuid;
    v_qty := (v_elem ->> 'qty')::int;
    v_color := v_elem ->> 'color';
    v_size := v_elem ->> 'size';
  ...
  for v_line in select * from t_order_lines
  loop
    ...
    if not found then
      raise exception 'Producto inexistente: %', v_line.product_id;
    end if;
    if v_available < v_line.qty then
      raise exception 'Stock insuficiente para %', v_product_name;
    end if;

    if v_line.color is not null and not exists (
      select 1 from public.product_colors c
      where c.product_id = v_line.product_id and c.name = v_line.color
    ) then
      raise exception 'Color inválido para %', v_product_name;
    end if;
    if v_line.size is not null and not exists (
      select 1 from public.product_sizes s
      where s.product_id = v_line.product_id and s.name = v_line.size
    ) then
  ...
  loop
    ...
    values (
      v_order_id, v_line.product_id, v_line.name, v_line.color, v_line.size,
      v_line.qty, v_line.unit_price, v_line.unit_price * v_line.qty
    );
    ...
    values (v_line.product_id, v_order_id, 'RESERVATION', v_line.qty, 'Reserva de stock al crear la orden ' || v_number);
  end loop;
